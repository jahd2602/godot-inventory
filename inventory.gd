...
export(String) var _name      = "Inventory"
...
class Slot:
	...
	func update_contents():
		...
		if stack != null:
			...
			stack.set_position(Vector2(1, 1))
			stack.layout()
		else:
			set_tooltip("")
	...
	func drop_data(p, v):
		if !add_stack(v[1]) && v[0].has_method("add_stack"):
			v[0].add_stack(remove_and_collide())
			add_stack(v[1])
	...
	func remove_and_collide(c = null):
		var s
	...
	func merge(s):
		if stack == null:
			set(s.remove_and_collide())
		elif stack.can_stack(s.get()):
			stack.stack(s.remove_and_collide())
		update_contents()
...
func _ready():
	...
	get_child(0).set_text(_name)
	for i in range(size):
		var slot = Slot.new(self)
		slot.set_name("slot_"+str(i))
		add_child(slot)
		slots.append(slot)
		slot.set_position(Vector2(6+(SLOT_SIZE + 5)*(i%5), 25+top_space+(SLOT_SIZE + 5)*(i/5)))
	var cols = 5
	if size < 5:
		cols = size
	var rows = (size-1)/5 + 1
...
func collapse(c):
	...
	for c in get_children():
		if !c.get_class() == "Control" || c.get_name() == "Label":
			continue
		if collapsed:
			c.hide()
		else:
...
func _gui_input(event):
	...
	if event is InputEventMouseButton && event.button_index == 1 && event.pressed:
		if CAN_DRAG_WINDOW:
			...
			collapse(!collapsed)
	elif moving && event is InputEventMouseMotion:
		if !Input.is_mouse_button_pressed(1):
			moving = false
		else:
			set_position(get_position()+event.position-old_mouse_pos)
			old_mouse_pos = event.position
	# right mouse button shows the menu
	elif !moving && event is InputEventMouseButton && event.button_index == 2:
		show_menu(get_global_transform().get_origin() + event.position)
	else:
		return
	accept_event()
	...
	for slot in slots:
		if slot.stack != null && slot.stack.item == itemdesc:
			if slot.stack.count > c:
				slot.remove_and_collide(c)
				return
			else:
				c -= slot.stack.count
				slot.remove_and_collide()
